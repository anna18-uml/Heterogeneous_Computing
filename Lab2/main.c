...
#error "unknown OpenCL SDK environment"
    ...
    kernel = clCreateKernel(program, "pi_cal", &ret);
    if (ret != CL_SUCCESS) {
    ...
    int id=0;
      cl_mem id_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY |
          CL_MEM_COPY_HOST_PTR, sizeof(id), &id, NULL);
     float *result=(float *)calloc(1,sizeof(float));
     
    cl_mem result_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE , sizeof(float),  NULL,&ret);

    ret = 0;
    /* Create kernel argument */
    ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), &id_buffer);
    ret |= clSetKernelArg(kernel, 1, sizeof(int), &num_to_work);
    ...
    printf("the value of series without multiplying with 4 is %f\n",*result);
    float pi=(*result)*4;
